...
public class ThreeSum {
	...
	public int[] searchRange(int[] nums, int start, int end, int target) {
		...
		// if we got out of the range of the array, that means we finished our search
		// and the target doesn't exist
		if (start < 0 || start > nums.length - 1 || end < 0 || end > nums.length - 1) {
			return new int[] { -1, -1 };
		}

		// if we shrank the range until we start equals end and target not equal to
		// current elem, return -1,-1
		...
		// start searching to get the min/max index, we may have duplicates of the
		// target in the left and right side of current index
		if (nums[mid] == target) {
			return findIndx(nums, mid, target);
		} else if (target < nums[mid]) {// if target less than current elem, eleminate the search range from start ..
										// mid - 1
			return searchRange(nums, start, mid - 1, target);
		} else {// if target greater than current elem, eleminate the search range from mid +1
				// .. end
	...
	int[] findIndx(int[] nums, int index, int target) {
		...
		int start = index;
		// loop from current index to left side until 0 and every time update start in
		// case current elem still equals target otherwise exit
		while (current >= 0) {
		...
		// loop from current index to right side until length-1 and every time update
		// start in case current elem still equals target otherwise exit
